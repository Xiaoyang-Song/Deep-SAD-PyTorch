...
class FashionMNIST_Dataset_Customized(TorchvisionDataset):
    ...
    def __init__(self, root: str, normal_class: int = 0, known_outlier_class: int = 1, 
        ...
        if sampler == "original":
            ...
            self.train_set = MyFashionMNIST(root=self.root, train=True, transform=transform, target_transform=target_transform, download=True)
            self.train_set.data = train_set
            self.train_set.targets = train_targets
            self.train_set.semi_targets = semi_targets

        # Get test set
        self.test_set = MyFashionMNIST(root=self.root, train=False, transform=transform, target_transform=target_transform, download=True)
